...
def test_all_cases_pass():
    ...
    for case in cases:
        ...
        expected = case["expected_output"] if "expected_output" in case else None
        expected_exc = case.get("expected_exception")
        try:
            output = apply_discounts(data)
        except Exception as exc:  # noqa: BLE001
            outcome = isinstance(exc, (DiscountProcessorError, json.JSONDecodeError)) and expected_exc is not None
        else:
            if expected_exc is not None:
                outcome = False
            else:
                outcome = abs(output - expected) < 1e-6

...
```powershell
cd Project_A_Faulty
.\setup_original.ps1  # One-time setup
.\run_original.ps1    # Run tests
```
...
Review `log_original.txt` for pytest output and `time_original.txt` for runtime metrics.
...
.\setup_optimized.ps1  # One-time setup
.\run_optimized.ps1    # Run tests
```
...
Inspect `log_optimized.txt` and `time_optimized.txt` for outcomes and timing.
...
- PowerShell scripts (`.ps1`) are provided for Windows; bash scripts (`.sh`) for Linux/Mac.
- Logs and performance reports are generated deterministically by the automation
  scripts after each run.

## Windows PowerShell Setup

If you encounter execution policy restrictions, enable script execution:
